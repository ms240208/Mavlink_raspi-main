# Log_RLS3.py
# Log_RLS2.py を参考に、デバッグメッセージ（補正量）もCSVに記録するバージョン


# Log_RLS2.pyのMAVLink受信・記録ロジックを参考に、PhaseCorrメッセージも記録
import re
import csv
import os
import signal
...
def parse_phasecorr_message(msg):
    pattern = r"PhaseCorr: slope=([\d\.-]+) ideal=([\d\.-]+) err=([\d\.-]+) corr=([\d\.-]+)"
...
def main():
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow([
            ...
            "Cold_Start_Progress",
            # PhaseCorr用
            "slope", "ideal_slope", "phase_error", "phase_correction", "PhaseCorr_raw"
        ])
        ...
        while running and not stop_event.is_set():
            ...
            if text.startswith("PhaseCorr:"):
                slope, ideal, err, corr = parse_phasecorr_message(text)
                phasecorr_cache = (slope, ideal, err, corr, text)
                # PhaseCorr単体でも記録（1Hzで来る想定）
                if record_event.is_set():
                    ts = datetime.now().strftime("%Y/%m/%d %H:%M:%S.%f")[:-3]
                    row = [ts] + [""]*15 + [slope, ideal, err, corr, text]
                    writer.writerow(row)
                    csvfile.flush()
                    print(f"{ts} : PhaseCorr slope={slope} ideal={ideal} err={err} corr={corr}")
                continue
            ...
            if record_event.is_set() and (f_curr and f_pred and abcA and abcB and abcC):
                ...
                if phasecorr_cache:
                    slope, ideal, err, corr, phasecorr_raw = phasecorr_cache
                else:
                    slope = ideal = err = corr = phasecorr_raw = ""
                row = [
                    ts,
                ...
                if record_count % 5 == 0:
                    print(f"{ts} : 記録 #{record_count} F_curr=({f_curr[0]:.3f},{f_curr[1]:.3f},{f_curr[2]:.3f}) "
                          ...
                          f"F_pred=({f_pred[0]:.3f},{f_pred[1]:.3f},{f_pred[2]:.3f}) PhaseCorr=({slope},{ideal},{err},{corr})")
                record_count += 1
                f_curr = f_pred = None
